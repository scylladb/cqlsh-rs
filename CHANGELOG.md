...
## [0.5.17](https://github.com/scylladb/cqlsh-rs/compare/v0.5.16...v0.5.17) - 2026-08-04

### Fixed

- *(pager)* stop mutating $PAGER in tests so cargo test cannot hang on a TTY

### Other

- *(homebrew)* point formula at v0.5.16 [skip ci]
...
- *(pager)* detect BusyBox less instead of assuming GNU flags

### Other

- test the Docker image end-to-end through a PTY
- commit Cargo.lock for reproducible/downstream builds
...
- use Python cqlsh-compatible tracing toggle messages
...
- suppress connection banner in non-interactive mode (-e, -f, pipe)
...
- *(ci)* download only needed artifacts in upload-release-artifacts job
